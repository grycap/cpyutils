...
def run_in_thread(host, port, use_ssl = False):
    _bottle_thr = threading.Thread(target=run, args=(host, port, use_ssl))
    _bottle_thr.daemon = True
    _bottle_thr.start()
    return _bottle_thr

def run(host, port, use_ssl = False):
    global _bottle_server, _app
    if use_ssl:
        # Add our new MySSLCherryPy class to the supported servers  
        # under the key 'mysslcherrypy'
        _bottle_server = MySSLCherryPy(host=host, port=port)
